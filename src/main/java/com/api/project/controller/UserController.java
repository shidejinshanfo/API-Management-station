...
public class UserController {
     ...
     * 上传信息
     *
     * @param userUploadRequest    自行上传的信息
     * @param request HttpServletRequest
     * @return BaseResponse<String>，返回头像的访问URL
     */
    @PostMapping("/upload")
    public BaseResponse<Boolean> uploadUser(@RequestBody UserUploadRequest userUploadRequest, HttpServletRequest request) {
        if (userUploadRequest == null || userUploadRequest.getId() == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        User user = new User();
        BeanUtils.copyProperties(userUploadRequest, user);
        boolean result = userService.updateById(user);
        return ResultUtils.success(result);
    }
